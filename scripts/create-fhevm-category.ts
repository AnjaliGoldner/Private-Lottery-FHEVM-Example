 ...
 *
 * Example: ts-node scripts/create-fhevm-category.ts applications ./output/fhevm-applications
 */
...
const CATEGORIES: Record<string, CategoryConfig> = {
  basic: {
    ...
    description:
      "Fundamental FHEVM operations including encrypted counters, storage, access control, and comparison operations",
    contracts: [
      {
      ...
      {
        path: "contracts/basic/EncryptedStorage.sol",
        test: "test/basic/EncryptedStorage.ts",
      },
      {
        path: "contracts/basic/AccessControlDemo.sol",
        test: "test/basic/AccessControlDemo.ts",
      },
      {
        path: "contracts/basic/ComparisonOperations.sol",
  ...
  decryption: {
    ...
    description:
      "User and public decryption patterns for accessing encrypted data",
    contracts: [
      {
        path: "contracts/decryption/UserDecryption.sol",
  ...
  bestpractices: {
    ...
    description:
      "Common mistakes to avoid and proper patterns for FHEVM development",
    contracts: [
      {
        path: "contracts/examples/AntiPatterns.sol",
  ...
  applications: {
    ...
    description:
      "Real-world applications demonstrating FHEVM capabilities including lotteries, voting, and auctions",
    contracts: [
      {
        path: "contracts/PrivateLottery.sol",
...
function createCategoryProject(category: string, outputDir: string): void {
  ...
  [".gitkeep"].forEach((file) => {
    const contractGitkeep = path.join(contractsDir, file);
    const testGitkeep = path.join(testsDir, file);
    if (fs.existsSync(contractGitkeep)) fs.unlinkSync(contractGitkeep);
...
function main(): void {
  ...
  if (args.length === 0 || args[0] === "--help" || args[0] === "-h") {
    ...
    log("  ts-node scripts/create-fhevm-category.ts applications ./output/applications");
    process.exit(0);
  }
